...
from models.blip import create_vit, init_tokenizer_tablip, load_checkpoint

class TABLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        if vit=="unitable-large": 
            self.visual_encoder, vision_width = create_vit(vit, image_size, vit_grad_ckpt, vit_ckpt_layer, 0)
            assert vit_ckpt_path is not None
            checkpoint = torch.load(vit_ckpt_path, map_location="cpu") 
        ...
        bert_ckpt_path = bert_ckpt_path if bert_ckpt_path is not None else 'bert-base-uncased'
        
        self.max_length = max_length
        self.tokenizer = init_tokenizer_tablip(bert_ckpt_path)

        encoder_config = BertConfig.from_json_file(med_config)
        encoder_config.encoder_width = vision_width
        self.text_encoder = BertModel(encoder_config, add_pooling_layer=False)
        ...
        self.load_bert_state(encoder_config, bert_ckpt_path)
        ...
        self.visual_encoder_m.interpolate_position_embeddings()
...
def tablip_pretrain(**kwargs):
    model = TABLIP_Pretrain(**kwargs)
    return model 
