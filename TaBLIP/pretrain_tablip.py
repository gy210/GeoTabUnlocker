...
from models.tablip_pretrain import tablip_pretrain
import utils
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    for i, (image, caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if epoch==0:
            warmup_lr_schedule(
                optimizer, i, config['warmup_steps'], config['warmup_lr'], config['init_lr'])
...
def main(args, config):
    ...
    datasets = [create_dataset('pretrain_tablip', config, min_scale=0.2)]
    ...
    data_loader = create_loader(
        datasets, samplers, batch_size=[config['batch_size']], 
        num_workers=[4], is_trains=[True], collate_fns=[None])[0]

    #### Model #### 
    print("Creating model")
    model = tablip_pretrain(
        image_size=config['image_size'], 
        max_length=config['max_length'],
        vit=config['vit'], 
        vit_ckpt_path=config["vit_ckpt_path"], 
        vit_grad_ckpt=config['vit_grad_ckpt'], 
        vit_ckpt_layer=config['vit_ckpt_layer'], 
        bert_ckpt_path=config["bert_ckpt_path"],
    ...
    for epoch in range(start_epoch, config['max_epoch']):
        ...
        step_lr_schedule(
            optimizer, epoch, config['init_lr'], config['min_lr'], config['lr_decay_rate'])
